...
CNDO2System::compute_cndo_f_matrix_internal(const arma::mat &p_alpha,
  ...
  for (const auto &atom : atoms_) {
    for (const auto &orbital : atom.get_atomic_orbitals()) {
      ...
      // second term (differs for alpha/beta): [(p^tot_AA - Z_A) - (p^alpha_{mu
      // mu} -1/2)]*gamma_AA
      double diag_term_a =
          diag_term + ((p_AA.at(iatom) - atom.get_atom_constant("Z_A")) -
                       (p_alpha(iorbital, iorbital) - 0.5)) *
                          gamma(iorbital, iorbital);
      double diag_term_b =
          diag_term + ((p_AA.at(iatom) - atom.get_atom_constant("Z_A")) -
                       (p_beta(iorbital, iorbital) - 0.5)) *
                          gamma(iorbital, iorbital);
...
arma::cube CNDO2System::compute_overlap_matrix_gradient() {
  ...
  arma::cube dS =
      arma::zeros(3, basis_functions.size(), basis_functions.size());

  // Iterate through pairs of orbitals - make sure to not compare orbitals in
  // the same atom
  for (const auto &[begin, end] : atom_orbital_idxs) {
    for (int i = begin; i < end; ++i) {
      for (int j = end; j < basis_functions.size(); ++j) {
        ...
        std::array<double, 3> local_gradient =
            integrate_product_dRa(orbital_i, orbital_j);
...
arma::cube CNDO2System::compute_gamma_matrix_gradient() {
  ...
  for (size_t iatom = 0; iatom < atoms_.size(); ++iatom) {
    ...
    for (size_t jatom = iatom + 1; jatom < atoms_.size(); ++jatom) {
      ...
      arma::vec3 term = calculate_gamma_dRa(atom_i.get_atomic_orbitals().at(0),
                                            atom_j.get_atomic_orbitals().at(0));
      for (size_t idim = 0; idim < 3; ++idim) {
...
arma::mat CNDO2System::E_electronic_dRA() {
  ...
  for (size_t iatom = 0; iatom < atoms_.size(); ++iatom) {
    ...
    for (size_t iao = atom_orbital_idxs[iatom][0];
         iao < atom_orbital_idxs[iatom][1]; ++iao) {
      for (size_t jatom = 0; jatom < atoms_.size(); ++jatom) {
        if (jatom == iatom)
          continue;

        double prefactor = (atom_i.get_atom_constant("neg_beta") +
                            atoms_.at(jatom).get_atom_constant("neg_beta"));

        for (size_t jao = atom_orbital_idxs[jatom][0];
             jao < atom_orbital_idxs[jatom][1]; ++jao) {
          for (size_t idim = 0; idim < 3; ++idim) {
            gradient(idim, iatom) += prefactor *
                                     (p_alpha_(iao, jao) + p_beta_(iao, jao)) *
                                     dS(idim, iao, jao);
          }
        }
      }
    ...
    for (size_t jatom = 0; jatom < atoms_.size(); ++jatom) {
      ...
      for (size_t iao = atom_orbital_idxs[iatom][0];
        ...
        for (size_t jao = atom_orbital_idxs[jatom][0];
             jao < atom_orbital_idxs[jatom][1]; ++jao) {
          prefactor -= p_alpha_(iao, jao) * p_alpha_(iao, jao) +
                       p_beta_(iao, jao) * p_beta_(iao, jao);
        }
      }
...
arma::mat CNDO2System::E_nuclear_dRA() {
  ...
  for (size_t iatom = 0; iatom < atoms_.size(); ++iatom) {
    const Atom &atom_i = atoms_.at(iatom);
    arma::vec3 RA;
    std::copy(std::begin(atom_i.get_position()),
              std::end(atom_i.get_position()), std::begin(RA));
    ...
    for (size_t jatom = iatom + 1; jatom < atoms_.size(); ++jatom) {
      ...
      arma::vec3 RB;
      std::copy(std::begin(atom_j.get_position()),
                std::end(atom_j.get_position()), std::begin(RB));
...
double
CNDO2System::get_electron_density(const std::array<double, 3> &position) const {
  double density = 0;
  arma::mat occupied_mos =
      arma::join_rows(get_occupied_MOs_alpha(), get_occupied_MOs_beta());
  for (int i = 0; i < 1; ++i) {
    ...
    for (int j = 0; j < occupied_mos.n_rows; ++j) {
      ...
      GaussianContracted ao =
          atoms_[icur_atom]
              .get_atomic_orbitals()[j - atom_orbital_idxs[icur_atom][0]];
...
arma::cube CNDO2System::get_electron_density_3d_grid(
    ...
    const std::array<double, 2> &z_range, int nsamples_per_side) const {
  ...
  arma::vec sample_x =
      arma::linspace(x_range[0], x_range[1], nsamples_per_side);
  arma::vec sample_y =
      arma::linspace(y_range[0], y_range[1], nsamples_per_side);
  arma::vec sample_z =
      arma::linspace(z_range[0], z_range[1], nsamples_per_side);

  arma::cube values =
      arma::cube(nsamples_per_side, nsamples_per_side, nsamples_per_side);
  arma::mat occupied_mos =
      arma::join_rows(get_occupied_MOs_alpha(), get_occupied_MOs_beta());
